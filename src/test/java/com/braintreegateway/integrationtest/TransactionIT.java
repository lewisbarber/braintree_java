...
public class TransactionIT extends IntegrationTest implements MerchantAccountTestConstants {
	...
	public void createViaTransparentRedirect() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().options().storeInVault(true).done();
	...
	public void createWithPaymentMethodNonce() {
		...
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).paymentMethodNonce(nonce);
	...
	public void cloneTransaction() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).orderId("123").creditCard().number(CreditCardNumber.VISA.number)
				.expirationDate("05/2009").done().customer().firstName("Dan").done().billingAddress().firstName("Carl").done().shippingAddress().firstName("Andrew").done();

		Result<Transaction> result = gateway.transaction().sale(request);
		assertTrue(result.isSuccess());
		Transaction transaction = result.getTarget();

		TransactionCloneRequest cloneRequest = new TransactionCloneRequest().amount(new BigDecimal("123.45")).channel("MyShoppingCartProvider").options().submitForSettlement(false)
				.done();
	...
	public void cloneTransactionWithValidationErrors() {
		...
		assertEquals(ValidationErrorCode.TRANSACTION_CANNOT_CLONE_CREDIT, cloneResult.getErrors().forObject("transaction").onField("base").get(0).getCode());
	}
	...
	public void saleWithAccessToken() {
		...
		OAuthCredentialsRequest oauthRequest = new OAuthCredentialsRequest().code(code).scope("read_write");
	...
	public void saleWithCardTypeIndicators() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard()
				.number(CreditCardNumbers.CardTypeIndicators.Prepaid.getValue()).expirationDate("05/2012").done();
	...
	public void saleWithAllAttributes() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).channel("MyShoppingCartProvider").orderId("123").creditCard()
				.cardholderName("The Cardholder").number(CreditCardNumber.VISA.number).cvv("321").expirationDate("05/2009").done().customer().firstName("Dan").lastName("Smith")
				.company("Braintree Payment Solutions").email("dan@example.com").phone("419-555-1234").fax("419-555-1235").website("http://braintreepayments.com").done()
				.billingAddress().firstName("Carl").lastName("Jones").company("Braintree").streetAddress("123 E Main St").extendedAddress("Suite 403").locality("Chicago")
				.region("IL").postalCode("60622").countryName("United States of America").countryCodeAlpha2("US").countryCodeAlpha3("USA").countryCodeNumeric("840").done()
				.shippingAddress().firstName("Andrew").lastName("Mason").company("Braintree Shipping").streetAddress("456 W Main St").extendedAddress("Apt 2F").locality("Bartlett")
				.region("MA").postalCode("60103").countryName("Mexico").countryCodeAlpha2("MX").countryCodeAlpha3("MEX").countryCodeNumeric("484").done();
	...
	public void saleWithSpecifyingMerchantAccountId() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(NON_DEFAULT_MERCHANT_ACCOUNT_ID).amount(TransactionAmount.AUTHORIZE.amount).creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").done();
	...
	public void saleWithStoreInVaultAndSpecifyingToken() {
		...
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().token(paymentToken).number(CreditCardNumber.VISA.number)
				.expirationDate("05/2009").done().customer().id(customerId).firstName("Jane").done().options().storeInVault(true).done();
	...
	public void saleWithStoreInVaultWithoutSpecifyingToken() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().customer().firstName("Jane").done().options().storeInVault(true).done();
	...
	public void saleWithStoreInVaultOnSuccessWhenTransactionSucceeds() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().customer().firstName("Jane").done().options().storeInVaultOnSuccess(true).done();
	...
	public void saleWithStoreInVaultOnSuccessWhenTransactionFails() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.DECLINE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().customer().firstName("Jane").done().options().storeInVaultOnSuccess(true).done();
	...
	public void saleWithStoreInVaultForBillingAndShipping() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().billingAddress().firstName("Carl").done().shippingAddress().firstName("Andrew").done().options().storeInVault(true).addBillingAddressToPaymentMethod(true)
	...
	public void saleWithVaultCustomerAndNewCreditCard() {
		Customer customer = gateway.customer().create(new CustomerRequest().firstName("Michael").lastName("Angelo").company("Some Company")).getTarget();

		TransactionRequest request = new TransactionRequest().amount(SandboxValues.TransactionAmount.AUTHORIZE.amount).customerId(customer.getId()).creditCard()
				.cardholderName("Bob the Builder").number(SandboxValues.CreditCardNumber.VISA.number).expirationDate("05/2009").done();
	...
	public void saleWithVaultCustomerAndNewCreditCardStoresInVault() {
		...
		TransactionRequest request = new TransactionRequest().amount(SandboxValues.TransactionAmount.AUTHORIZE.amount).customerId(customer.getId()).creditCard()
				.cardholderName("Bob the Builder").number(SandboxValues.CreditCardNumber.VISA.number).expirationDate("05/2009").done().options().storeInVault(true).done();
	...
	public void saleWithApplePayNonce() {
		...
		TransactionRequest request = new TransactionRequest().amount(SandboxValues.TransactionAmount.AUTHORIZE.amount).paymentMethodNonce(applePayNonce);
	...
	public void saleWithAndroidPayProxyCardNonce() {
		...
		TransactionRequest request = new TransactionRequest().amount(SandboxValues.TransactionAmount.AUTHORIZE.amount).paymentMethodNonce(androidPayCardNonce);
	...
	public void saleWithAmexExpressCheckoutCardNonce() {
		...
		TransactionRequest request = new TransactionRequest().merchantAccountId(FAKE_AMEX_DIRECT_MERCHANT_ACCOUNT_ID).amount(SandboxValues.TransactionAmount.AUTHORIZE.amount)
	...
	public void saleWithVenmoAccountNonce() {
		...
		TransactionRequest request = new TransactionRequest().merchantAccountId(FAKE_VENMO_ACCOUNT_MERCHANT_ACCOUNT_ID).amount(SandboxValues.TransactionAmount.AUTHORIZE.amount)
	...
	public void saleWithThreeDSecureOptionRequired() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(THREE_D_SECURE_MERCHANT_ACCOUNT_ID).amount(TransactionAmount.AUTHORIZE.amount).creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").done().options().threeDSecure().required(true).done().done();
	...
	public void saleWithThreeDSecureToken() {
		String threeDSecureToken = TestHelper.createTest3DS(gateway, THREE_D_SECURE_MERCHANT_ACCOUNT_ID,
				new ThreeDSecureRequestForTests().number(CreditCardNumber.VISA.number).expirationMonth("05").expirationYear("2009"));

		TransactionRequest request = new TransactionRequest().merchantAccountId(THREE_D_SECURE_MERCHANT_ACCOUNT_ID).amount(TransactionAmount.AUTHORIZE.amount)
				.threeDSecureToken(threeDSecureToken).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009").done();
	...
	public void saleErrorWithNullThreeDSecureToken() {
		...
		Result<Transaction> result = gateway.transaction().sale(request);
		assertFalse(result.isSuccess());
		assertEquals(ValidationErrorCode.TRANSACTION_THREE_D_SECURE_TOKEN_IS_INVALID, result.getErrors().forObject("transaction").onField("threeDSecureToken").get(0).getCode());
	}
	...
	public void saleErrorWithMismatchedThreeDSecureData() {
		...
		TransactionRequest request = new TransactionRequest().merchantAccountId(THREE_D_SECURE_MERCHANT_ACCOUNT_ID).amount(TransactionAmount.AUTHORIZE.amount)
				.threeDSecureToken(threeDSecureToken).creditCard().number(CreditCardNumber.MASTER_CARD.number).expirationDate("05/2009").done();
	...
	public void saleWithAmexRewards() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(FAKE_AMEX_DIRECT_MERCHANT_ACCOUNT_ID).amount(TransactionAmount.AUTHORIZE.amount).creditCard()
				.number(CreditCardNumber.AmexPayWithPoints.SUCCESS.number).expirationDate("12/2020").done().options().submitForSettlement(true).amexRewards().requestId("ABC123")
				.points("1000").currencyAmount("10.00").currencyIsoCode("USD").done().done();
	...
	public void saleWithAmexRewardsSucceedsEvenIfCardIneligible() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(FAKE_AMEX_DIRECT_MERCHANT_ACCOUNT_ID).amount(TransactionAmount.AUTHORIZE.amount).creditCard()
				.number(CreditCardNumber.AmexPayWithPoints.INELIGIBLE_CARD.number).expirationDate("12/2020").done().options().submitForSettlement(true).amexRewards()
				.requestId("ABC123").points("1000").currencyAmount("10.00").currencyIsoCode("USD").done().done();
	...
	public void saleWithAmexRewardsSucceedsEvenIfCardBalanceIsInsufficient() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(FAKE_AMEX_DIRECT_MERCHANT_ACCOUNT_ID).amount(TransactionAmount.AUTHORIZE.amount).creditCard()
				.number(CreditCardNumber.AmexPayWithPoints.INSUFFICIENT_POINTS.number).expirationDate("12/2020").done().options().submitForSettlement(true).amexRewards()
				.requestId("ABC123").points("1000").currencyAmount("10.00").currencyIsoCode("USD").done().done();
	...
	public void submitForSettlementWithAmexRewards() {
		...
		TransactionRequest request = new TransactionRequest().merchantAccountId(FAKE_AMEX_DIRECT_MERCHANT_ACCOUNT_ID).amount(new BigDecimal("100.00")).creditCard()
				.number(CreditCardNumber.AmexPayWithPoints.SUCCESS.number).expirationDate("12/2020").done().options().amexRewards().requestId("ABC123").points("1000")
				.currencyAmount("10.00").currencyIsoCode("USD").done().done();
	...
	public void submitForSettlementWithAmexRewardsSucceedsEvenIfCardIsIneligible() {
		...
		TransactionRequest request = new TransactionRequest().merchantAccountId(FAKE_AMEX_DIRECT_MERCHANT_ACCOUNT_ID).amount(new BigDecimal("100.00")).creditCard()
				.number(CreditCardNumber.AmexPayWithPoints.INELIGIBLE_CARD.number).expirationDate("12/2020").done().options().amexRewards().requestId("ABC123").points("1000")
				.currencyAmount("10.00").currencyIsoCode("USD").done().done();
	...
	public void submitForSettlementWithAmexRewardsSucceedsEvenIfCardBalanceIsInsufficient() {
		...
		TransactionRequest request = new TransactionRequest().merchantAccountId(FAKE_AMEX_DIRECT_MERCHANT_ACCOUNT_ID).amount(new BigDecimal("100.00")).creditCard()
				.number(CreditCardNumber.AmexPayWithPoints.INSUFFICIENT_POINTS.number).expirationDate("12/2020").done().options().amexRewards().requestId("ABC123").points("1000")
				.currencyAmount("10.00").currencyIsoCode("USD").done().done();
	...
	public void saleWithFraudCardIsDeclined() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.FRAUD.number)
				.expirationDate("05/2016").done();
	...
	public void saleWithSecurityParams() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).deviceSessionId("abc123").fraudMerchantId("456").creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").done();
	...
	public void saleWithCustomFields() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).customField("storeMe", "custom value")
				.customField("another_stored_field", "custom value2").creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009").done();
		Result<Transaction> result = gateway.transaction().sale(request);
		assertTrue(result.isSuccess());
		Transaction transaction = result.getTarget();
	...
	public void saleReturnsCreditCardPaymentInstrumentType() {
		...
		assertEquals(PaymentInstrumentType.CREDIT_CARD, transaction.getPaymentInstrumentType());
	}

	@Test
	public void saleWithRecurringFlag() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).recurring(true).creditCard().number(CreditCardNumber.VISA.number)
				.expirationDate("05/2009").done();
		Result<Transaction> result = gateway.transaction().sale(request);
		assertTrue(result.isSuccess());
		Transaction transaction = result.getTarget();
	...
	public void saleWithValidationErrorsOnAddress() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.DECLINE.amount).customField("unkown_custom_field", "custom value").creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").done().billingAddress().countryName("No such country").countryCodeAlpha2("zz")
				.countryCodeAlpha3("zzz").countryCodeNumeric("000").done();
	...
	public void saleWithUnregisteredCustomField() {
		...
		assertEquals(ValidationErrorCode.TRANSACTION_CUSTOM_FIELD_IS_INVALID, result.getErrors().forObject("transaction").onField("customFields").get(0).getCode());
	}

	@Test
	public void saleWithMultipleValidationErrorsOnSameField() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).paymentMethodToken("foo").customerId("5").creditCard()
				.number(CreditCardNumber.VISA.number).cvv("321").expirationDate("04/2009").done();
	...
	public void saleWithCustomerId() {
		...
		CreditCardRequest creditCardRequest = new CreditCardRequest().customerId(customer.getId()).cvv("123").number("5105105105105100").expirationDate("05/12");
		CreditCard creditCard = gateway.creditCard().create(creditCardRequest).getTarget();

		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).paymentMethodToken(creditCard.getToken());
	...
	public void saleWithPaymentMethodTokenAndCvv() {
		Customer customer = gateway.customer().create(new CustomerRequest()).getTarget();
		CreditCardRequest creditCardRequest = new CreditCardRequest().customerId(customer.getId()).number("5105105105105100").expirationDate("05/12");
		CreditCard creditCard = gateway.creditCard().create(creditCardRequest).getTarget();

		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).paymentMethodToken(creditCard.getToken()).creditCard().cvv("301").done();
	...
	public void saleUsesShippingAddressFromVault() {
		...
		Address shippingAddress = gateway.address().create(customer.getId(), new AddressRequest().firstName("Carl")).getTarget();

		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).customerId(customer.getId()).shippingAddressId(shippingAddress.getId());
	...
	public void saleUsesBillingAddressFromVault() {
		...
		Address billingAddress = gateway.address().create(customer.getId(), new AddressRequest().firstName("Carl")).getTarget();

		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).customerId(customer.getId()).billingAddressId(billingAddress.getId());
	...
	public void saleWithValidationError() {
		TransactionRequest request = new TransactionRequest().amount(null).creditCard().expirationMonth("05").expirationYear("2010").done();
	...
	public void saleWithDescriptor() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().descriptor().name("123*123456789012345678").phone("3334445555").url("ebay.com").done();
	...
	public void saleWithDescriptorValidation() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().descriptor().name("badcompanyname12*badproduct12").phone("%bad4445555").url("12345678901234").done();

		Result<Transaction> result = gateway.transaction().sale(request);
		assertFalse(result.isSuccess());

		assertEquals(ValidationErrorCode.DESCRIPTOR_NAME_FORMAT_IS_INVALID, result.getErrors().forObject("transaction").forObject("descriptor").onField("name").get(0).getCode());

		assertEquals(ValidationErrorCode.DESCRIPTOR_PHONE_FORMAT_IS_INVALID, result.getErrors().forObject("transaction").forObject("descriptor").onField("phone").get(0).getCode());

		assertEquals(ValidationErrorCode.DESCRIPTOR_URL_FORMAT_IS_INVALID, result.getErrors().forObject("transaction").forObject("descriptor").onField("url").get(0).getCode());
	}

	@Test
	public void saleWithLodgingIndustryData() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().industry().industryType(Transaction.IndustryType.LODGING).data().folioNumber("aaa").checkInDate("2014-07-07").checkOutDate("2014-08-08").roomRate("2.00")
				.done().done();
	...
	public void saleWithIndustryDataValidation() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().industry().industryType(Transaction.IndustryType.LODGING).data().folioNumber("aaa").checkInDate("2014-07-07").checkOutDate("2014-06-06").done().done();
	...
	public void saleWithTravelCruiseIndustryData() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().industry().industryType(Transaction.IndustryType.TRAVEL_CRUISE).data().travelPackage("flight").departureDate("2014-07-07").lodgingCheckInDate("2014-07-07")
				.lodgingCheckOutDate("2014-08-08").lodgingName("Disney").done().done();
	...
	public void saleWithTravelCruiseIndustryDataValidation() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().industry().industryType(Transaction.IndustryType.TRAVEL_CRUISE).data().travelPackage("plane").departureDate("2014-07-07").lodgingCheckInDate("2014-07-07")
				.lodgingCheckOutDate("2014-08-08").lodgingName("Disney").done().done();
	...
	public void saleWithLevel2() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().taxAmount(new BigDecimal("10.00")).taxExempt(true).purchaseOrderNumber("12345");
	...
	public void saleWithTooLongPurchaseOrderNumber() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().purchaseOrderNumber("aaaaaaaaaaaaaaaaaa");
	...
	public void saleWithInvalidPurchaseOrderNumber() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().purchaseOrderNumber("\u00c3\u009f\u00c3\u00a5\u00e2\u0088\u0082");

		Result<Transaction> result = gateway.transaction().sale(request);
		assertFalse(result.isSuccess());

		assertEquals(ValidationErrorCode.TRANSACTION_PURCHASE_ORDER_NUMBER_IS_INVALID, result.getErrors().forObject("transaction").onField("purchaseOrderNumber").get(0).getCode());
	}

	@Test
	public void saleWithVenmoSdkPaymentMethodCode() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).venmoSdkPaymentMethodCode(VenmoSdk.PaymentMethodCode.Visa.code);
	...
	public void saleWithVenmoSdkSession() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.done().options().venmoSdkSession(VenmoSdk.Session.Valid.value).done();
	...
	public void createTransactionFromTransparentRedirectWithAddress() {
		...
		TransactionRequest trParams = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).type(Transaction.Type.SALE).creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").done().billingAddress().countryName("United States of America").countryCodeAlpha2("US")
				.countryCodeAlpha3("USA").countryCodeNumeric("840").done();
	...
	public void createTransactionFromTransparentRedirectWithAddressWithErrors() {
		...
		TransactionRequest trParams = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).type(Transaction.Type.SALE).creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").done().billingAddress().countryName("Foo bar!").countryCodeAlpha2("zz").countryCodeAlpha3("zzz")
				.countryCodeNumeric("000").done();
	...
	@Test
	public void credit() {
		TransactionRequest request = new TransactionRequest().amount(SandboxValues.TransactionAmount.AUTHORIZE.amount).creditCard()
	...
	public void creditWithSpecifyingMerchantAccountId() {
		TransactionRequest request = new TransactionRequest().amount(SandboxValues.TransactionAmount.AUTHORIZE.amount).merchantAccountId(NON_DEFAULT_MERCHANT_ACCOUNT_ID)
				.creditCard().number(SandboxValues.CreditCardNumber.VISA.number).expirationDate("05/2009").done();
	...
	public void creditWithCustomFields() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).customField("store_me", "custom value")
				.customField("another_stored_field", "custom value2").creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009").done();
		Result<Transaction> result = gateway.transaction().credit(request);
		assertTrue(result.isSuccess());
		Transaction transaction = result.getTarget();
	...
	@Test
	public void find() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2008")
				.done();
	...
	public void voidWithBadStatus() {
		...
		assertFalse(result.isSuccess());
		assertEquals(ValidationErrorCode.TRANSACTION_CANNOT_BE_VOIDED, result.getErrors().forObject("transaction").onField("base").get(0).getCode());
	}
	...
	public void submitForSettlementWithOrderId() {
		...
		TransactionRequest submitForSettlementRequest = new TransactionRequest().orderId("1234");
	...
	public void submitForSettlementWithDescriptors() {
		...
		TransactionRequest submitForSettlementRequest = new TransactionRequest().descriptor().name("123*123456789012345678").phone("3334445555").url("ebay.com").done();
	...
	public void submitForSettlementWithBadStatus() {
		...
		assertFalse(result.isSuccess());
		assertEquals(ValidationErrorCode.TRANSACTION_CANNOT_SUBMIT_FOR_SETTLEMENT, result.getErrors().forObject("transaction").onField("base").get(0).getCode());
	}
	...
	public void updateDetails() {
		...
		TransactionRequest updateDetailsRequest = new TransactionRequest().amount(new BigDecimal("123.45")).descriptor().name("123*123456789012345678").phone("3334445555")
				.url("ebay.com").done().orderId("1234");
	...
	public void updateDetailsWithInvalidAmount() {
		...
		TransactionRequest updateDetailsRequest = new TransactionRequest().amount(new BigDecimal("9999")).descriptor().name("123*123456789012345678").phone("3334445555")
				.url("ebay.com").done().orderId("1234");

		Result<Transaction> result = gateway.transaction().updateDetails(transaction.getId(), updateDetailsRequest);

		assertFalse(result.isSuccess());
		assertEquals(ValidationErrorCode.TRANSACTION_SETTLEMENT_AMOUNT_IS_TOO_LARGE, result.getErrors().forObject("transaction").onField("amount").get(0).getCode());
	}
	...
	public void updateDetailsWithInvalidDescriptor() {
		...
		TransactionRequest updateDetailsRequest = new TransactionRequest().descriptor().name("invalid name").phone("invalid phone").url("invalid url is way too long to be valid")
				.done().orderId("1234");
	...
	public void updateDetailsWithInvalidProcessor() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(FAKE_AMEX_DIRECT_MERCHANT_ACCOUNT_ID).amount(TransactionAmount.AUTHORIZE.amount).creditCard()
				.number(CreditCardNumber.AmexPayWithPoints.SUCCESS.number).expirationDate("12/2020").done().options().submitForSettlement(true).done();
		...
		assertFalse(result.isSuccess());
		assertEquals(ValidationErrorCode.TRANSACTION_PROCESSOR_DOES_NOT_SUPPORT_UPDATING_DETAILS, result.getErrors().forObject("transaction").onField("base").get(0).getCode());
	}
	...
	public void searchOnAllTextFields() {
		...
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("1000")).creditCard().number("4111111111111111").expirationDate("05/2012")
				.cardholderName("Tom Smith").token(creditCardToken).done().billingAddress().company("Braintree").countryName("United States of America")
				.extendedAddress("Suite 123").firstName(firstName).lastName("Smith").locality("Chicago").postalCode("12345").region("IL").streetAddress("123 Main St").done()
				.customer().company("Braintree").email("smith@example.com").fax("5551231234").firstName("Tom").lastName("Smith").phone("5551231234").website("http://example.com")
				.done().options().storeInVault(true).submitForSettlement(true).done().orderId("myorder").shippingAddress().company("Braintree P.S.").countryName("Mexico")
				.extendedAddress("Apt 456").firstName("Thomas").lastName("Smithy").locality("Braintree").postalCode("54321").region("MA").streetAddress("456 Road").done();

		Transaction transaction = gateway.transaction().sale(request).getTarget();
		TestHelper.settle(gateway, transaction.getId());
		transaction = gateway.transaction().find(transaction.getId());

		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).billingCompany().is("Braintree").billingCountryName()
				.is("United States of America").billingExtendedAddress().is("Suite 123").billingFirstName().is(firstName).billingLastName().is("Smith").billingLocality()
				.is("Chicago").billingPostalCode().is("12345").billingRegion().is("IL").billingStreetAddress().is("123 Main St").creditCardCardholderName().is("Tom Smith")
				.creditCardExpirationDate().is("05/2012").creditCardNumber().is(CreditCardNumber.VISA.number).currency().is("USD").customerCompany().is("Braintree").customerEmail()
				.is("smith@example.com").customerFax().is("5551231234").customerFirstName().is("Tom").customerId().is(transaction.getCustomer().getId()).customerLastName()
				.is("Smith").customerPhone().is("5551231234").customerWebsite().is("http://example.com").orderId().is("myorder").paymentMethodToken().is(creditCardToken)
				.processorAuthorizationCode().is(transaction.getProcessorAuthorizationCode()).settlementBatchId().is(transaction.getSettlementBatchId()).shippingCompany()
				.is("Braintree P.S.").shippingCountryName().is("Mexico").shippingExtendedAddress().is("Apt 456").shippingFirstName().is("Thomas").shippingLastName().is("Smithy")
				.shippingLocality().is("Braintree").shippingPostalCode().is("54321").shippingRegion().is("MA").shippingStreetAddress().is("456 Road");
	...
	public void searchOnTextNodeOperators() {
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("1000")).creditCard().number("4111111111111111").expirationDate("05/2012")
				.cardholderName("Tom Smith").done();
		Transaction transaction = gateway.transaction().sale(request).getTarget();

		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).creditCardCardholderName().startsWith("Tom");
		...
		searchRequest = new TransactionSearchRequest().user().is("integration_user_public_id").id().is(transaction.getId());

		collection = gateway.transaction().search(searchRequest);
		assertEquals(1, collection.getMaximumSize());

		searchRequest = new TransactionSearchRequest().creditCardUniqueIdentifier().is(transaction.getCreditCard().getUniqueNumberIdentifier()).id().is(transaction.getId());
	...
	public void searchOnsPaymentInstrumentTypeIsCreditCard() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).paymentInstrumentType().is("CreditCardDetail");
	...
	public void searchOnsPaymentInstrumentTypeIsPayPal() {
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("1000")).paymentMethodNonce(Nonce.PayPalFuturePayment);

		Transaction transaction = gateway.transaction().sale(request).getTarget();

		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).paymentInstrumentType().is("PayPalDetail");
	...
	public void searchOnsPaymentInstrumentTypeIsApplePay() {
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("1000")).paymentMethodNonce(Nonce.ApplePayVisa);

		Transaction transaction = gateway.transaction().sale(request).getTarget();

		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).paymentInstrumentType().is("ApplePayDetail");
	...
	public void searchOnPaymentInstrumentTypeIsEuropeBank() {
		BraintreeGateway altpayGateway = new BraintreeGateway(Environment.DEVELOPMENT, "altpay_merchant", "altpay_merchant_public_key", "altpay_merchant_private_key");
		...
		TransactionRequest request = new TransactionRequest().merchantAccountId("fake_sepa_ma").amount(TransactionAmount.AUTHORIZE.amount).paymentMethodNonce(nonce);

		Transaction transaction = altpayGateway.transaction().sale(request).getTarget();

		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).paymentInstrumentType().is("EuropeBankAccountDetail");
	...
	public void searchOnNullValue() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).creditCardCardholderName().is(null);
	...
	public void searchOnCreatedUsing() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2010")
				.done();

		Transaction transaction = gateway.transaction().sale(request).getTarget();

		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).createdUsing().is(Transaction.CreatedUsing.FULL_INFORMATION);
	...
	public void searchOnCreditCardCustomerLocation() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).creditCardCustomerLocation().is(CreditCard.CustomerLocation.US);

		ResourceCollection<Transaction> collection = gateway.transaction().search(searchRequest);

		assertEquals(1, collection.getMaximumSize());

		searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).creditCardCustomerLocation().in(CreditCard.CustomerLocation.US,
	...
	public void searchOnMerchantAccountId() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).merchantAccountId().is(transaction.getMerchantAccountId());
	...
	public void searchOnCreditCardType() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).creditCardCardType().is(CreditCard.CardType.VISA);
	...
	public void searchOnEuropeBankAccountIban() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).europeBankAccountIban().is("DE89370400440532013000");
	...
	public void searchOnStatus() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).status().is(Transaction.Status.AUTHORIZED);
		...
		searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).status().is(Transaction.Status.SETTLEMENT_CONFIRMED);

		collection = gateway.transaction().search(searchRequest);

		assertEquals(0, collection.getMaximumSize());

		searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).status().is(Transaction.Status.SETTLEMENT_DECLINED);
	...
	public void searchOnAuthorizationExpiredStatus() {
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().status().is(Transaction.Status.AUTHORIZATION_EXPIRED);
		ResourceCollection<Transaction> collection = gateway.transaction().search(searchRequest);
	...
	public void searchOnSource() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).source().is(Transaction.Source.API);
		...
		searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).source().in(Transaction.Source.API, Transaction.Source.RECURRING);

		collection = gateway.transaction().search(searchRequest);

		assertEquals(1, collection.getMaximumSize());

		searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).source().in(Transaction.Source.RECURRING);
	...
	public void searchOnType() {
		...
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2010")
				.cardholderName(name).done().options().submitForSettlement(true).done();
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().creditCardCardholderName().is(name).type().is(Transaction.Type.CREDIT);
	...
	public void searchOnAmount() {
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("1000")).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2010").done();

		Transaction transaction = gateway.transaction().sale(request).getTarget();

		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).amount().between(new BigDecimal("500"), new BigDecimal("1500"));
	...
	public void searchOnDisbursementDate() throws ParseException {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(DISBURSEMENT_TRANSACTION_ID).disbursementDate().between(oneDayEarlier, oneDayLater);
	...
	public void searchOnDisputeDate() throws ParseException {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(DISPUTED_TRANSACTION_ID).disputeDate().between(oneDayEarlier, oneDayLater);

		assertEquals(1, gateway.transaction().search(searchRequest).getMaximumSize());

		searchRequest = new TransactionSearchRequest().id().is(TWO_DISPUTE_TRANSACTION_ID).disputeDate().greaterThanOrEqualTo(oneDayEarlier);

		assertEquals(1, gateway.transaction().search(searchRequest).getMaximumSize());

		searchRequest = new TransactionSearchRequest().id().is(DISPUTED_TRANSACTION_ID).disputeDate().lessThanOrEqualTo(oneDayLater);

		assertEquals(1, gateway.transaction().search(searchRequest).getMaximumSize());

		searchRequest = new TransactionSearchRequest().id().is(DISPUTED_TRANSACTION_ID).disputeDate().between(threeDaysEarlier, oneDayEarlier);
	...
	public void searchOnCreatedAt() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).createdAt().between(oneDayEarlier, oneDayLater);
	...
	public void searchOnAuthorizationExpiredAt() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().authorizationExpiredAt().between(oneDayEarlier, oneDayLater);
	...
	public void searchOnAuthorizedAt() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).authorizedAt().between(oneDayEarlier, oneDayLater);
	...
	public void searchOnFailedAt() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.FAILED.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2010")
				.done();
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).failedAt().between(oneDayEarlier, oneDayLater);
	...
	public void searchOnGatewayRejectedAt() {
		BraintreeGateway processingRulesGateway = new BraintreeGateway(Environment.DEVELOPMENT, "processing_rules_merchant_id", "processing_rules_public_key",
				"processing_rules_private_key");
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2009")
				.cvv("200").done();
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).gatewayRejectedAt().between(oneDayEarlier, oneDayLater);

		assertEquals(1, processingRulesGateway.transaction().search(searchRequest).getMaximumSize());

		searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).gatewayRejectedAt().greaterThanOrEqualTo(oneDayEarlier);

		assertEquals(1, processingRulesGateway.transaction().search(searchRequest).getMaximumSize());

		searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).gatewayRejectedAt().lessThanOrEqualTo(oneDayLater);

		assertEquals(1, processingRulesGateway.transaction().search(searchRequest).getMaximumSize());

		searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).gatewayRejectedAt().between(threeDaysEarlier, oneDayEarlier);
	...
	public void searchOnProcessorDeclinedAt() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.DECLINE.amount).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2010")
				.done();
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).processorDeclinedAt().between(oneDayEarlier, oneDayLater);
	...
	public void searchOnSettledAt() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).settledAt().between(oneDayEarlier, oneDayLater);
	...
	public void searchOnSubmittedForSettlementAt() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).submittedForSettlementAt().between(oneDayEarlier, oneDayLater);
	...
	public void searchOnVoidedAt() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).voidedAt().between(oneDayEarlier, oneDayLater);
	...
	public void searchOnMultipleStatusAtFields() {
		...
		searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).authorizedAt().between(threeDaysEarlier, oneDayEarlier).submittedForSettlementAt()
				.between(threeDaysEarlier, oneDayEarlier);
	...
	public void searchOnPayPalFields() {
		...
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId()).paypalPaymentId().startsWith("PAY").paypalPayerEmail()
				.is("payer@example.com").paypalAuthorizationId().startsWith("SALE");
	...
	public void searchReturnsAndHandlesInvalidCriteria() {
		TransactionSearchRequest searchRequest = new TransactionSearchRequest().amount().is(new BigDecimal("-500"));
	...
	public void refundFailsWithNonSettledTransaction() {
		...
		assertEquals(ValidationErrorCode.TRANSACTION_CANNOT_REFUND_UNLESS_SETTLED, result.getErrors().forObject("transaction").onField("base").get(0).getCode());
	}
	...
	public void gatewayRejectedOnApplicationIncomplete() {
		...
		MerchantRequest request = new MerchantRequest().email("name@email.com").countryCodeAlpha3("USA").paymentMethods(Arrays.asList("credit_card", "paypal"));
		...
		TransactionRequest transactionRequest = new TransactionRequest().amount(new BigDecimal(4000.00)).creditCard().number(CreditCardNumber.VISA.number).expirationDate("05/2020")
				.done();
	...
	public void gatewayRejectedOnAvs() {
		BraintreeGateway processingRulesGateway = new BraintreeGateway(Environment.DEVELOPMENT, "processing_rules_merchant_id", "processing_rules_public_key",
				"processing_rules_private_key");
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).billingAddress().postalCode("20001").done().creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").done();
	...
	public void gatewayRejectedOnAvsAndCvv() {
		BraintreeGateway processingRulesGateway = new BraintreeGateway(Environment.DEVELOPMENT, "processing_rules_merchant_id", "processing_rules_public_key",
				"processing_rules_private_key");
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).billingAddress().postalCode("20001").done().creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").cvv("200").done();
	...
	public void snapshotPlanIdAddOnsAndDiscountsFromSubscription() {
		CustomerRequest customerRequest = new CustomerRequest().creditCard().number("5105105105105100").expirationDate("05/12").done();
		CreditCard creditCard = gateway.customer().create(customerRequest).getTarget().getCreditCards().get(0);

		SubscriptionRequest request = new SubscriptionRequest().paymentMethodToken(creditCard.getToken()).planId(PlanFixture.PLAN_WITHOUT_TRIAL.getId()).addOns().add()
				.amount(new BigDecimal("11.00")).inheritedFromId("increase_10").numberOfBillingCycles(5).quantity(2).done().add().amount(new BigDecimal("21.00"))
				.inheritedFromId("increase_20").numberOfBillingCycles(6).quantity(3).done().done().discounts().add().amount(new BigDecimal("7.50")).inheritedFromId("discount_7")
				.neverExpires(true).quantity(2).done().done();
	...
	public void serviceFee() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(NON_DEFAULT_SUB_MERCHANT_ACCOUNT_ID).amount(new BigDecimal("100.00")).creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").done().serviceFeeAmount(new BigDecimal("1.00"));
	...
	public void serviceFeeNotAllowedForMasterMerchant() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(NON_DEFAULT_MERCHANT_ACCOUNT_ID).amount(new BigDecimal("100.00")).creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2017").done().serviceFeeAmount(new BigDecimal("1.00"));
	...
	public void holdInEscrowOnCreate() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(NON_DEFAULT_SUB_MERCHANT_ACCOUNT_ID).amount(new BigDecimal("100.00")).creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2009").done().serviceFeeAmount(new BigDecimal("1.00")).options().holdInEscrow(true).done();
		Result<Transaction> result = gateway.transaction().sale(request);
		assertTrue(result.isSuccess());
		assertEquals(Transaction.EscrowStatus.HOLD_PENDING, result.getTarget().getEscrowStatus());
	}
	...
	public void holdInEscrowOnSaleForMasterMerchantAccount() {
		...
		Result<Transaction> result = gateway.transaction().sale(request);
		assertFalse(result.isSuccess());
		assertEquals(ValidationErrorCode.TRANSACTION_CANNOT_HOLD_IN_ESCROW, result.getErrors().forObject("transaction").onField("base").get(0).getCode());
	}

	@Test
	public void holdInEscrowAfterSale() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(NON_DEFAULT_SUB_MERCHANT_ACCOUNT_ID).amount(new BigDecimal("100.00")).creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2012").done().serviceFeeAmount(new BigDecimal("1.00"));
		...
		assertEquals(Transaction.EscrowStatus.HOLD_PENDING, holdInEscrow.getTarget().getEscrowStatus());
	}

	@Test
	public void holdInEscrowAfterSaleFailsForMasterMerchants() {
		TransactionRequest request = new TransactionRequest().merchantAccountId(NON_DEFAULT_MERCHANT_ACCOUNT_ID).amount(new BigDecimal("100.00")).creditCard()
				.number(CreditCardNumber.VISA.number).expirationDate("05/2012").done();
		Result<Transaction> sale = gateway.transaction().sale(request);
		assertTrue(sale.isSuccess());
		Result<Transaction> holdInEscrow = gateway.transaction().holdInEscrow(sale.getTarget().getId());
		assertFalse(holdInEscrow.isSuccess());
		assertEquals(ValidationErrorCode.TRANSACTION_CANNOT_HOLD_IN_ESCROW, holdInEscrow.getErrors().forObject("transaction").onField("base").get(0).getCode());
	}
	...
	public void releaseFromEscrow() {
		...
		assertEquals(Transaction.EscrowStatus.RELEASE_PENDING, releaseResult.getTarget().getEscrowStatus());
	}
	...
	public void releaseFromEscrowFailsWhenTransactionIsNotEscrowed() {
		...
		assertEquals(ValidationErrorCode.TRANSACTION_CANNOT_RELEASE_FROM_ESCROW, releaseResult.getErrors().forObject("transaction").onField("base").get(0).getCode());
	}
	...
	public void cancelReleaseSucceeds() {
		...
		assertEquals(Transaction.EscrowStatus.HELD, cancelResult.getTarget().getEscrowStatus());
	}
	...
	public void cancelReleaseFailsReleasingNonPendingTransactions() {
		...
		assertEquals(ValidationErrorCode.TRANSACTION_CANNOT_CANCEL_RELEASE, cancelResult.getErrors().forObject("transaction").onField("base").get(0).getCode());
	}

	@Test
	public void createOneTimePayPalTransaction() {
		String nonce = TestHelper.generateOneTimePayPalNonce(gateway);
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("100.00")).paymentMethodNonce(nonce);
		...
		assertEquals(PaymentInstrumentType.PAYPAL_ACCOUNT, saleResult.getTarget().getPaymentInstrumentType());
	}

	@Test
	public void createPayPalTransactionWithPayeeEmail() {
		String nonce = TestHelper.generateOneTimePayPalNonce(gateway);
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("100.00")).paymentMethodNonce(nonce).paypalAccount().payeeEmail("payee@example.com").done();
	...
	public void createPayPalTransactionWithPayeeEmailInOptionsParams() {
		String nonce = TestHelper.generateOneTimePayPalNonce(gateway);
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("100.00")).paymentMethodNonce(nonce).paypalAccount().done().options()
				.payeeEmail("payee@example.com").done();
	...
	public void createPayPalTransactionWithPayeeEmailInOptionsPayPalParams() {
		String nonce = TestHelper.generateOneTimePayPalNonce(gateway);
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("100.00")).paymentMethodNonce(nonce).paypalAccount().done().options().paypal()
				.payeeEmail("payee@example.com").done().done();
	...
	public void createPayPalTransactionWithPayPalCustomField() {
		String nonce = TestHelper.generateOneTimePayPalNonce(gateway);
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("100.00")).paymentMethodNonce(nonce).paypalAccount().done().options().paypal()
				.customField("custom field stuff").done().done();
	...
	public void createPayPalTransactionWithPayPalSupplementaryData() {
		String nonce = TestHelper.generateOneTimePayPalNonce(gateway);
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("100.00")).paymentMethodNonce(nonce).paypalAccount().done().options().paypal()
				.supplementaryData("key1", "value1").supplementaryData("key2", "value2").done().done();
	...
	public void createPayPalTransactionWithPayPalDescription() {
		String nonce = TestHelper.generateOneTimePayPalNonce(gateway);
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("100.00")).paymentMethodNonce(nonce).paypalAccount().done().options().paypal()
				.description("Product description").done().done();
	...
	public void createOneTimePayPalTransactionAndAttemptToVault() {
		String nonce = TestHelper.generateOneTimePayPalNonce(gateway);
		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("100.00")).paymentMethodNonce(nonce).options().storeInVault(true).done();
	...
	public void createPayPalTransactionFromVaultRecord() {
		...
		PaymentMethodRequest vaultRequest = new PaymentMethodRequest().customerId(customer.getId()).paymentMethodNonce(nonce);

		Result<? extends PaymentMethod> vaultResult = gateway.paymentMethod().create(vaultRequest);
		assertTrue(vaultResult.isSuccess());

		TransactionRequest request = new TransactionRequest().amount(new BigDecimal("100.00")).paymentMethodToken(vaultResult.getTarget().getToken());
	...
	public void refundPayPalTransaction() {
		String nonce = TestHelper.generateOneTimePayPalNonce(gateway);
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).paymentMethodNonce(nonce).options().submitForSettlement(true).done();
		Transaction transaction = gateway.transaction().sale(request).getTarget();
		TestHelper.settle(gateway, transaction.getId());
	...
	public void paypalTransactionReturnsSettlementResponseCode() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).paymentMethodNonce(Nonce.PayPalFuturePayment).options()
	...
	public void settleAltPayTransaction() {
		...
		TransactionRequest request = new TransactionRequest().merchantAccountId("fake_sepa_ma").amount(TransactionAmount.AUTHORIZE.amount).paymentMethodNonce(nonce).options()
				.submitForSettlement(true).done();

		Transaction transaction = altpayGateway.transaction().sale(request).getTarget();
		TestHelper.settle(altpayGateway, transaction.getId());

		TransactionSearchRequest searchRequest = new TransactionSearchRequest().id().is(transaction.getId());
	...
	public void successfulPartialSettlementSale() {
		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).creditCard().number(CreditCardNumber.MASTER_CARD.number)
				.expirationDate("05/2009").done();
	...
	public void submitForPartialSettlementWithOrderId() {
		...
		TransactionRequest submitForPartialSettlementRequest = new TransactionRequest().amount(amount).orderId("1234");
	...
	public void submitForPartialSettlementWithDescriptors() {
		...
		BigDecimal amount = new BigDecimal("400.00");
		TransactionRequest submitForPartialSettlementRequest = new TransactionRequest().amount(amount).descriptor().name("123*123456789012345678").phone("3334445555")
				.url("ebay.com").done();
	...
	public void sharedPaymentMethods() {
		BraintreeGateway sharerGateway = new BraintreeGateway(Environment.DEVELOPMENT, "integration_merchant_public_id", "oauth_app_partner_user_public_key",
				"oauth_app_partner_user_private_key");
		Customer customer = sharerGateway.customer()
				.create(new CustomerRequest().creditCard().number("5105105105105100").expirationDate("05/19").billingAddress().postalCode("94107").done().done()).getTarget();
		CreditCard card = customer.getCreditCards().get(0);
		Address billingAddress = card.getBillingAddress();
		Address shippingAddress = sharerGateway.address().create(customer.getId(), new AddressRequest().postalCode("94107")).getTarget();

		BraintreeGateway oauthGateway = new BraintreeGateway("client_id$development$integration_client_id", "client_secret$development$integration_client_secret");
		String code = TestHelper.createOAuthGrant(oauthGateway, "integration_merchant_id", "shared_vault_transactions");

		OAuthCredentialsRequest oauthRequest = new OAuthCredentialsRequest().code(code).scope("shared_vault_transactions");

		Result<OAuthCredentials> accessTokenResult = oauthGateway.oauth().createTokenFromCode(oauthRequest);

		BraintreeGateway gateway = new BraintreeGateway(accessTokenResult.getTarget().getAccessToken());

		TransactionRequest request = new TransactionRequest().amount(TransactionAmount.AUTHORIZE.amount).sharedPaymentMethodToken(card.getToken())
				.sharedCustomerId(customer.getId()).sharedShippingAddressId(shippingAddress.getId()).sharedBillingAddressId(billingAddress.getId());
